...
async def test_toxicity_evaluation(monocle_trace_asserter: TraceAssertion):
    travel_request = "Book a flight from San Francisco to Mumbai for 26th April 2026. Book a two queen room at Marriott Intercontinental at Central Mumbai for 27th April 2026 for 4 nights."
    crew = create_crewai_travel_crew(travel_request)
    await monocle_trace_asserter.run_agent_async(crew, "crewai", travel_request)                
    monocle_trace_asserter.called_tool("book_flight","Flight Booking Agent")
    monocle_trace_asserter.called_tool("book_hotel","Hotel Booking Agent") 
